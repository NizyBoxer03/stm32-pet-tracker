...
/* USER CODE BEGIN PV */
uint8_t ATCommand[20];
uint8_t data_rx2[BUF_SIZE];
uint8_t data_rx1[BUF_SIZE];
uint8_t data_tx[BUF_SIZE];

typedef struct
{
	int hour, minute, latdeg, longdeg, day, month, year, checksum;
	float second, latmin, longmin, speed, tracktrue, magvar;
	char valid, latdir, longdir, magvardir, mode;
} GPS_DATA; //Para almacenar datos de GPRMC, después lo movemos
